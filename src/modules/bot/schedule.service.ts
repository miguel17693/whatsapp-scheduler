        ...
        for (const msg of this.scheduledMessages) {
            if (msg.sendAt <= now) {
                try {
                    ...
                    this.logger.log(`Attempting to send scheduled message to ${msg.to}`, this.client.info);
                    ...
                    if (retries < this.MAX_RETRIES) {
                        this.logger.error(`Failed to send message to ${msg.to} (attempt ${retries + 1}/${this.MAX_RETRIES}): ${error.message}`, this.client.info);
                        messagesToKeep.push({
                            ...msg,
